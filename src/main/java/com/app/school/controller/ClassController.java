...
import com.app.school.service.ClassService;
...
public class ClassController {
    ...
    @GetMapping("/find")
    public ResponseEntity<?> findClassByTitle(@RequestParam(required = false, defaultValue = "") String title) {
        List<ClassDTO>  collection;

        if (StringUtils.isEmpty(title)) {
            collection = classService.findAll().stream().map(ClassDTO::new)
                    .collect(Collectors.toList());

            return new ResponseEntity<>(collection, HttpStatus.OK);
        }
        ClassEntity classEntity = classService.findByTitle(title);

        return new ResponseEntity<>(new ClassDTO(classEntity), HttpStatus.OK);
    }
}
